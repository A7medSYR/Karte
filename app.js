...
function startTracking() {
  ...
  console.log("Starte Geolocation-Tracking");
  ...
  try {
    ...
    console.error('Fehler beim Starten der Geolocation:', e);
  }
}
...
function updatePosition(position) {
  ...
  // Höhere Toleranz für Genauigkeit (300m statt 100m)
  if (accuracy > 200) {
    console.warn('Position hat niedrige Genauigkeit:', accuracy);
    // Trotzdem weiter verarbeiten, nicht mehr verwerfen
  }
  ...
  // Ausreißer erkennen aber mit höherer Toleranz (500m statt 100m)
  if (positionHistory.length > 1) {
    ...
    if (dist > 500) {
      console.warn('Großer Positionssprung erkannt:', dist, 'Meter');
      // Trotzdem weiter verarbeiten, nicht verwerfen
    }
  }
  ...
  if (tracking) {
    console.log('Zentriere Karte auf Position:', filteredPosition);
  ...
  if (!userMarker) {
    console.log('Erstelle neuen Benutzermarker');
...
function setupUI() {
  ...
  // Stelle sicher, dass Standortverfolgung explizit gestartet wird
  if (!tracking || !watchId) {
    startTracking();
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Stelle sicher, dass Tracking gestartet wird
  setTimeout(() => {
    if (!watchId) {
      startTracking();
    }
