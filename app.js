...
function startTracking() {
  if (watchId) {
    // Stoppe zuerst den vorherigen Watch, um Konflikte zu vermeiden
  ...
  // Debug-Meldung für Geolocation-Verfügbarkeit
  if ('geolocation' in navigator) {
   
  } else {
    console.error('Geolocation ist nicht verfügbar');
    alert('Ihr Browser unterstützt keine Standorterkennung.');
    return;
  }

  // Versuche Geolocation zu starten
  try {
    ...
    alert('Fehler beim Starten der Standortverfolgung: ' + e.message);
  }
}
...
function setupUI() {
  ...
  if (!tracking || !watchId) {
    console.log("Starte Standortverfolgung beim Setup");
    startTracking();
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => {
    if (!watchId) {
      console.log('Starte Standortverfolgung erneut nach Timeout');
      startTracking();
    }
  }, 1000);
