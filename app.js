...
// Globale Variablen für das Routing
    ...
    reader.onload = function(e) {
      ...
      Papa.parse(cleanedData, {
        ...
        complete: async results => {
          ...
          // Marker-Referenzen für die Routenberechnung speichern
          allMarkers = markerRefs;

          // "Route berechnen" Button hinzufügen
          const routeBtn = document.createElement('button');
          routeBtn.innerHTML = 'Route berechnen';
          routeBtn.id = 'calculate-route-btn';
          ...
          `;
          routeBtn.addEventListener('click', calculateRouteToAllStops);
          
          // Bestehenden Button entfernen falls vorhanden
          const existingBtn = document.getElementById('calculate-route-btn');
          if (existingBtn) {
            existingBtn.remove();
          }
...
// Neue Funktion: Berechnet die beste Route zu allen Stops
async function calculateRouteToAllStops() {
  // Prüfen ob wir Marker haben
  if (allMarkers.length === 0) {
    alert('Es wurden keine Stops gefunden. Bitte importieren Sie zuerst Adressen.');
    return;
  }

  // Prüfen ob wir einen aktuellen Standort haben
  if (!lastPosition) {
    alert('Ihr Standort konnte nicht bestimmt werden. Bitte aktivieren Sie die Standortfreigabe.');
    return;
  }
  ...
  loadingIndicator.onAdd = function() {
    const div = L.DomUtil.create('div', 'loading-indicator');
    div.innerHTML = '<div style="background: white; padding: 5px; border-radius: 4px; display: inline-block;">Route wird berechnet...</div>';
  ...
  try {
    // 1. Extrahiere alle Koordinaten der Marker
    const coordinates = [];
    coordinates.push(lastPosition.latlng); // Startpunkt ist der aktuelle Standort
    
    // Extrahiere Koordinaten aus allen Markern
    allMarkers.forEach(markerObj => {
      const position = markerObj.marker.getLatLng();
      coordinates.push([position.lat, position.lng]);
    });

    // 2. Berechne die optimierte Route mit OSRM
    const waypoints = coordinates.map(coord => `${coord[1]},${coord[0]}`).join(';');
    const routeUrl = `https://router.project-osrm.org/trip/v1/driving/${waypoints}?source=first&roundtrip=false&overview=full&steps=true&geometries=geojson`;
    ...
    if (routeData.code !== 'Ok') {
      throw new Error(`Routing-Fehler: ${routeData.message || 'Unbekannter Fehler'}`);
    }

    // 3. Extrahiere die optimierte Reihenfolge
    const waypoint_order = routeData.waypoints.map(wp => wp.waypoint_index).slice(1); // ohne Startpunkt
    
    // Speichere optimierte Stops
    optimizedStops = waypoint_order.map(index => allMarkers[index - 1]); // -1 weil Startpunkt an Position 0 ist

    // 4. Zeichne die Route auf der Karte
    if (routePolyline) {
      map.removeLayer(routePolyline);
    }

    const routeCoordinates = routeData.trips[0].geometry.coordinates;
    const reversedCoordinates = routeCoordinates.map(coord => [coord[1], coord[0]]);
    
    routePolyline = L.polyline(reversedCoordinates, {
      color: '#4285F4',
      weight: 5,
      opacity: 0.7
    }).addTo(map);

    // 5. Passe die Karte an, um die gesamte Route anzuzeigen
    map.fitBounds(routePolyline.getBounds(), { padding: [50, 50] });

    // 6. Nummeriere die Marker entsprechend der Route
    currentRouteIndex = 0;
    
    optimizedStops.forEach((markerObj, index) => {
      // Aktualisiere das Popup mit der Routennummer
      const marker = markerObj.marker;
      const oldPopupContent = marker.getPopup().getContent();
      
      // Extrahiere den vorhandenen Content vor der Checkbox
      const contentBeforeCheckbox = oldPopupContent.split('<label')[0];
      
      // Erstelle neuen Popup-Inhalt mit Routennummer
      const newPopupContent = 
        `<div style="font-size: 1.2em; font-weight: bold; color: #4285F4; margin-bottom: 5px;">Stop #${index + 1}</div>` +
        contentBeforeCheckbox +
        `<label style="display:inline-flex;align-items:center;margin-top:6px;">
          <input type="checkbox" id="${markerObj.markerId}" style="margin-right:6px;"> Pin grün
        </label>`;
      
      marker.setPopupContent(newPopupContent);
    });

    // 7. Zeige Routeninformationen
    showRouteInfo(routeData.trips[0]);
    
    // 8. Füge Navigations-UI hinzu
    createNavigationUI();
    ...
    const loadingElement = document.getElementById('loading-route');
    if (loadingElement) {
      loadingElement.remove();
    }
  }
...
function createNavigationUI() {
  // Entferne vorhandenes UI falls es existiert
  const existingNav = document.getElementById('navigation-ui');
  if (existingNav) {
    existingNav.remove();
  }
  ...
  prevButton.style.cssText = `
    ...
    cursor: pointer;
  `;
  prevButton.addEventListener('click', navigateToPrevStop);
  ...
  nextButton.style.cssText = `
    ...
    cursor: pointer;
  `;
  nextButton.addEventListener('click', navigateToNextStop);
  
  const statusText = document.createElement('div');
  statusText.id = 'nav-status';
  statusText.innerHTML = `Stop 1/${optimizedStops.length}`;
  ...
  // Initial zum ersten Stop navigieren
  navigateToStop(0);
}

// Zum vorherigen Stop navigieren
function navigateToPrevStop() {
  if (currentRouteIndex > 0) {
    navigateToStop(currentRouteIndex - 1);
  }
}

// Zum nächsten Stop navigieren
function navigateToNextStop() {
  if (currentRouteIndex < optimizedStops.length - 1) {
    navigateToStop(currentRouteIndex + 1);
  }
}

// Zu einem bestimmten Stop navigieren
function navigateToStop(index) {
  if (index < 0 || index >= optimizedStops.length) return;
  ...
  if (statusText) {
    statusText.innerHTML = `Stop ${index + 1}/${optimizedStops.length}`;
  }
  
  // Zum Stop zoomen
  const marker = optimizedStops[index].marker;
...
function showRouteInfo(trip) {
  ...
  infoControl.onAdd = function() {
    ...
    div.innerHTML = `
      <div style="background: white; padding: 8px; border-radius: 4px; margin-bottom: 10px; box-shadow: 0 1px 5px rgba(0,0,0,0.2);">
        <div style="font-weight: bold; margin-bottom: 5px;">Routeninformation</div>
        <div>Distanz: ${distanceKm} km</div>
        <div>Dauer: ${durationMin} min</div>
        <div>Anzahl Stops: ${optimizedStops.length}</div>
      </div>
    `;
    div.id = 'route-info';
    return div;
