...
async function geocode(strasse, hausnr, ort) {
  try {
    ...
    if (directResult) {
      ...
      cache[key] = resultObj;
      console.log("Geocode-Ergebnis:", resultObj);
      return resultObj;
    }
    ...
    if (!directResult && hasLetters && numericPart) {
      ...
      if (cache[reducedKey]) {
        ...
        cache[key] = resultObj;
        console.log("Reduziertes Geocode-Ergebnis (aus Cache):", resultObj);
        return resultObj;
      }
      ...
      if (reducedResult) {
        ...
        console.log("Reduziertes Geocode-Ergebnis:", resultObj);
        return resultObj;
      }
      ...
      if (normalizedStrasse !== strasse) {
        ...
        if (fallbackResult) {
          ...
          cache[key] = resultObj;
          console.log("Fallback Geocode-Ergebnis:", resultObj);
          return resultObj;
        }
      }
    }
...
async function addAddressMarkers(addresses) {
  console.log("Verarbeite", addresses.length, "Adressen");
  ...
  statusDiv.innerHTML = `<div>Adressen werden geladen...</div><div>0/${addresses.length}</div>`;
  ...
  for (let i = 0; i < addresses.length; i++) {
    ...
    statusDiv.innerHTML = `<div>Adressen werden geladen...</div><div>${i+1}/${addresses.length}</div>`;
    ...
    try {
      ...
      if (locationResult && locationResult.coordinates) {
        // Koordinaten aus dem result extrahieren
        const lat = locationResult.coordinates[0];
        const lng = locationResult.coordinates[1];
        ...
        // Bei reduzierten Hausnummern Hinweis hinzufügen
        if (locationResult.isReduced) {
          popupContent += `<p><em style="color:#d9534f;">Hausnummer ${locationResult.originalHausnr} nicht gefunden, 
          zeige stattdessen Hausnummer ${locationResult.reducedHausnr}.</em></p>`;
        }
        ...
        console.warn(`Konnte Adresse nicht geocodieren: ${strasse} ${hausnummer}, ${ort}`);
        failCount++;
      }
    } catch (error) {
    ...
    // Kurze Pause, um Ratenlimits zu vermeiden
  ...
  resultDiv.style.boxShadow = '0 0 10px rgba(0, 0, 0, 0.3)';
  resultDiv.style.zIndex = '9999';
  resultDiv.style.textAlign = 'center';
  resultDiv.innerHTML = `
    <h3 style="margin-top:0">Adressen geladen</h3>
    <p><strong style="color:#4CAF50">${successCount}</strong> Adressen erfolgreich geladen</p>
    ${failCount > 0 ? `<p><strong style="color:#f44336">${failCount}</strong> Adressen konnten nicht gefunden werden</p>` : ''}
    <button id="close-result" style="background:#2196F3; color:white; border:none; padding:8px 15px; border-radius:4px; cursor:pointer">OK</button>
  `;
  ...
  if (window.addressMarkers.length > 0) {
    console.log("Zoome auf", window.addressMarkers.length, "Marker");
    const group = L.featureGroup(window.addressMarkers);
    map.fitBounds(group.getBounds(), { padding: [50, 50] });
  } else {
    alert("Keine Adressen gefunden. Bitte überprüfen Sie die CSV-Datei.");
  }
  
  console.log(`Adressverarbeitung abgeschlossen: ${successCount} erfolgreich, ${failCount} fehlgeschlagen`);
}