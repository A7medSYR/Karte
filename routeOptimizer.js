...
function displayRoute(route) {
  ...
  statusDiv.innerHTML = `<div>Berechne Straßenroute...</div><div>Das kann einen Moment dauern</div>`;
...
async function calculateOSRMRoutes(route) {
  ...
  for (let i = 0; i < route.length - 1; i++) {
    ...
    document.getElementById('routing-status').innerHTML = 
      `<div>Berechne Straßenroute...</div><div>Segment ${i+1}/${route.length-1}</div>`;
...
function addRouteLegend(route, isFallback = false) {
  ...
  if (!document.querySelector('.route-summary-fixed')) {
    ...
    summaryDiv.innerHTML = `<span style="color:#FF5722;">${stopCount}</span> Stops | ${(totalDistance/1000).toFixed(2)} km${isFallback ? ' (Luft)' : ''}`;
    document.body.appendChild(summaryDiv);
  } else {
    document.querySelector('.route-summary-fixed').innerHTML = 
      `<span style="color:#FF5722;">${stopCount}</span> Stops | ${(totalDistance/1000).toFixed(2)} km${isFallback ? ' (Luft)' : ''}`;
  }
